...
class YOLOv7p6HeadModule(YOLOv5HeadModule):
    ...
    def init_weights(self):
        ...
        super(YOLOv5HeadModule, self).init_weights()
        for mi, aux, s in zip(
            self.main_convs_pred, self.aux_convs_pred, self.featmap_strides
        ):  # from
            mi = mi[2]  # nn.Conv2d
            b = mi.bias.data.view(3, -1)
            # obj (8 objects per 640 image)
    ...
    def forward(self, x: Tuple[Tensor]) -> Tuple[List]:
        ...
        return multi_apply(
            self.forward_single, x, self.main_convs_pred, self.aux_convs_pred
        )

    def forward_single(
        self, x: Tensor, convs: nn.Module, aux_convs: Optional[nn.Module]
    ) -> Tuple[Union[Tensor, List], Union[Tensor, List], Union[Tensor, List]]:
        ...
        if not self.training or not self.use_aux:
            ...
            aux_pred_map = aux_pred_map.view(
                bs, self.num_base_priors, self.num_out_attrib, ny, nx
            )
            ...
            return (
                [cls_score, aux_cls_score],
                [bbox_pred, aux_bbox_pred],
...
class YOLOv7Head(YOLOv5Head):
    ...
    def loss_by_feat(
        ...
        if isinstance(cls_scores[0], Sequence):
            ...
            head_preds = self._merge_predict_results(
                bbox_preds_main, objectnesses_main, cls_scores_main
            )
            ...
            head_preds = self._merge_predict_results(
                bbox_preds, objectnesses, cls_scores
            )
    ...
    def _calc_loss(
        ...
        for i, head_pred in enumerate(head_preds):
            ...
            if num_pred_positive == 0:
                ...
                loss_obj += (
                    self.loss_obj(head_pred[..., 4], target_obj)
            ...
            # calc obj loss
            target_obj[batch_inds, proir_idx, grid_y, grid_x] = (
                iou.detach().clamp(0).type(target_obj.dtype)
            )
            loss_obj += (
                self.loss_obj(head_pred[..., 4], target_obj) * self.obj_level_weights[i]
            )
            ...
            if self.num_classes > 1:
                ...
                target_class = torch.full_like(
                    head_pred_positive[:, 5:], 0.0, device=device
                )
                target_class[range(num_pred_positive), pred_cls_scores] = 1.0
    ...
    def _merge_predict_results(
        ...
        """
        head_preds = []
        for bbox_pred, objectness, cls_score in zip(
            bbox_preds, objectnesses, cls_scores
        ):
            ...
            head_pred = (
                torch.cat([bbox_pred, objectness, cls_score], dim=2)
                .permute(0, 1, 3, 4, 2)
                .contiguous()
            )
