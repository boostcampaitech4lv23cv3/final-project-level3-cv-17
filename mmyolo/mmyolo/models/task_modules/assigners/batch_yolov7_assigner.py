...
class BatchYOLOv7Assigner(nn.Module):
    ...
    def forward(
        ...
        if batch_targets_normed.shape[1] == 0:
            ...
            return dict(
                mlvl_positive_infos=[pred_results[0].new_empty((0, 4))] * num_levels,
        ...
        mlvl_positive_infos, mlvl_priors, mlvl_targets_normed = self.simota_assigner(
            pred_results,
            batch_targets_normed,
            mlvl_positive_infos,
            mlvl_priors,
    ...
    def yolov5_assigner(
        ...
        for i in range(len(pred_results)):  # lever
            ...
            match_inds = (
                torch.max(wh_ratio, 1.0 / wh_ratio).max(2)[0] < self.prior_match_thr
            )
            batch_targets_scaled = batch_targets_scaled[
                match_inds
            ]  # (num_matched_target, 7)
            ...
            left, up = (
                (batch_targets_cxcy % 1 < near_neighbor_thr) & (batch_targets_cxcy > 1)
            ).T
            right, bottom = ((grid_xy % 1 < near_neighbor_thr) & (grid_xy > 1)).T
            offset_inds = torch.stack((torch.ones_like(left), left, up, right, bottom))
            batch_targets_scaled = batch_targets_scaled.repeat((5, 1, 1))[
                offset_inds
            ]  # ()
            retained_offsets = grid_offset.repeat(1, offset_inds.shape[1], 1)[
                offset_inds
            ]
    ...
    def simota_assigner(
        ...
        for batch_idx in range(pred_results[0].shape[0]):
            ...
            for i, head_pred in enumerate(pred_results):
                ...
                pred_positive_cxcy = (
                    pred_positive[:, :2].sigmoid() * 2.0 - 0.5 + grid
                ) * self.featmap_strides[i]
                pred_positive_wh = (
                    (pred_positive[:, 2:4].sigmoid() * 2) ** 2
                    * priors
                    * self.featmap_strides[i]
                )
            ...
            # scaled xywh
            batch_input_shape_wh = (
                pred_results[0].new_tensor(batch_input_shape[::-1]).repeat((1, 2))
            )
            ...
            gt_cls_per_image = (
                F.one_hot(targets_normed[:, 1].to(torch.int64), self.num_classes)
                .float()
                .unsqueeze(1)
                .repeat(1, num_pred_positive, 1)
            )
            # cls_score * obj
            cls_preds_ = _mlvl_obj_cls[:, 1:].unsqueeze(0).repeat(
                num_gts, 1, 1
            ) * _mlvl_obj_cls[:, 0:1].unsqueeze(0).repeat(num_gts, 1, 1)
            y = cls_preds_.sqrt_()
            ...
            top_k, _ = torch.topk(
                pair_wise_iou, min(self.candidate_topk, pair_wise_iou.shape[1]), dim=1
            )
