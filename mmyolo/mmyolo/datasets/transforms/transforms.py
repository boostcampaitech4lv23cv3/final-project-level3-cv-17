...
class LetterResize(MMDET_Resize):
    ...
    def _resize_masks(self, results: dict):
        ...
        # resize the gt_masks
        gt_mask_height = results["gt_masks"].height * results["scale_factor"][1]
        gt_mask_width = results["gt_masks"].width * results["scale_factor"][0]
        ...
        if len(gt_masks) == 0:
            padded_masks = np.empty((0, *results["img_shape"][:2]), dtype=np.uint8)
        else:
...
class YOLOv5RandomAffine(BaseTransform):
    ...
    def _get_random_homography_matrix(
        ...
        trans_x = (
            random.uniform(
                0.5 - self.max_translate_ratio, 0.5 + self.max_translate_ratio
            )
            * width
        )
        trans_y = (
    ...
    def filter_gt_bboxes(
        self, origin_bboxes: HorizontalBoxes, wrapped_bboxes: HorizontalBoxes
    ) -> torch.Tensor:
        """Filter gt bboxes.
        ...
        aspect_ratio = np.maximum(
            wrapped_w / (wrapped_h + 1e-16), wrapped_h / (wrapped_w + 1e-16)
        )
        ...
        area_valid_idx = (
            wrapped_w * wrapped_h / (origin_w * origin_h + 1e-16) > self.min_area_ratio
        )
...
class PPYOLOERandomDistort(BaseTransform):
    ...
    def __init__(
        ...
        assert 0 < self.num_distort_func <= 4, "num_distort_func must > 0 and <= 4"
        for cfg in [
            self.hue_cfg,
            self.saturation_cfg,
            self.contrast_cfg,
            ...
            assert 0.0 <= cfg["prob"] <= 1.0, "prob must >=0 and <=1"

    def transform_hue(self, results):
        """Transform hue randomly."""
        if random.uniform(0.0, 1.0) >= self.hue_cfg["prob"]:
            return results
        img = results["img"]
        ...
        rgb2yiq_matrix = np.array(
            [[0.114, 0.587, 0.299], [-0.321, -0.274, 0.596], [0.311, -0.523, 0.211]]
        )
        yiq2rgb_matric = np.array(
            [[1.0, -1.107, 1.705], [1.0, -0.272, -0.647], [1.0, 0.956, 0.621]]
        )
    ...
    def transform_saturation(self, results):
        ...
        if random.uniform(0.0, 1.0) >= self.saturation_cfg["prob"]:
            return results
        img = results["img"]
        delta = random.uniform(self.saturation_cfg["min"], self.saturation_cfg["max"])
    ...
    def transform_contrast(self, results):
        ...
        if random.uniform(0.0, 1.0) >= self.contrast_cfg["prob"]:
            return results
        img = results["img"]
        delta = random.uniform(self.contrast_cfg["min"], self.contrast_cfg["max"])
        img *= delta
        results["img"] = img
        return results
    ...
    def transform_brightness(self, results):
        ...
        if random.uniform(0.0, 1.0) >= self.brightness_cfg["prob"]:
            return results
        img = results["img"]
        delta = random.uniform(self.brightness_cfg["min"], self.brightness_cfg["max"])
        img += delta
        results["img"] = img
        return results
    ...
    def transform(self, results: dict) -> dict:
        ...
        functions = [
            self.transform_brightness,
            self.transform_contrast,
            self.transform_saturation,
            self.transform_hue,
        ]
...
class PPYOLOERandomCrop(BaseTransform):
    ...
    def __init__(
        self,
        aspect_ratio: List[float] = [0.5, 2.0],
        thresholds: List[float] = [0.0, 0.1, 0.3, 0.5, 0.7, 0.9],
        scaling: List[float] = [0.3, 1.0],
        num_attempts: int = 50,
    ...
    def _crop_data(
        self, results: dict, crop_box: Tuple[int, int, int, int], valid_inds: np.ndarray
    ) -> Union[dict, None]:
    ...
    def transform(self, results: dict) -> Union[dict, None]:
        ...
        Returns:
            dict: The result dict.
        """
        if results.get("gt_bboxes", None) is None or len(results["gt_bboxes"]) == 0:
            return results
    ...
    def _get_crop_size(self, image_size: Tuple[int, int]) -> Tuple[int, int]:
        ...
        if self.aspect_ratio is not None:
            ...
            aspect_ratio = random.uniform(max(min_ar, scale**2), min(max_ar, scale**-2))
    ...
    def _iou_matrix(
        self, gt_bbox: HorizontalBoxes, crop_bbox: np.ndarray, eps: float = 1e-10
    ) -> np.ndarray:
        ...
        overlap = np.prod(rightbottom - lefttop, axis=2) * (lefttop < rightbottom).all(
            axis=2
        )
    ...
    def _get_valid_inds(
        self, gt_bbox: HorizontalBoxes, img_crop_bbox: np.ndarray
    ) -> np.ndarray:
        ...
        valid = np.logical_and(
            img_crop_bbox[:2] <= centers, centers < img_crop_bbox[2:]
        ).all(axis=1)
        valid = np.logical_and(
