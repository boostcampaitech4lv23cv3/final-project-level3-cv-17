...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        help="Redefine area rules,but no more than three numbers." " e.g., 30 70 125",
    )
    parser.add_argument(
        "--func",
        default=None,
...
def show_bbox_num(cfg, out_dir, fig_set, class_name, class_num):
    ...
    # Draw designs
    fig = plt.figure(figsize=(fig_set["figsize"][0], fig_set["figsize"][1]), dpi=300)
...
def show_bbox_wh(out_dir, fig_set, class_bbox_w, class_bbox_h, class_name):
    ...
    ax.violinplot(class_bbox_w, positions_w, showmeans=True, showmedians=True, widths=4)
    ax.violinplot(class_bbox_h, positions_h, showmeans=True, showmedians=True, widths=4)
    ...
    out_name = fig_set["out_name"]
    fig.savefig(
        f"{out_dir}/{out_name}_bbox_wh.jpg", bbox_inches="tight", pad_inches=0.1
    )  # Save Image
    plt.close()
...
def show_bbox_wh_ratio(out_dir, fig_set, class_name, class_bbox_ratio):
    ...
    ax.violinplot(
        class_bbox_ratio, positions, showmeans=True, showmedians=True, widths=5
    )
    ...
    out_name = fig_set["out_name"]
    fig.savefig(
        f"{out_dir}/{out_name}_bbox_ratio.jpg", bbox_inches="tight", pad_inches=0.1
    )  # Save Image
    plt.close()
...
def show_bbox_area(out_dir, fig_set, area_rule, class_name, bbox_area_num):
    ...
    for i in range(len(area_rule) - 1):
        ...
        plt.bar(
            positions + width * i, area_num, width, label=labels[i], color=colors[i]
        )
        for idx, (x, y) in enumerate(zip(positions.tolist(), area_num)):
            plt.text(x + width * i, y, y, ha="center", fontsize=fig_set["fontsize"] - 1)
    ...
    plt.xlabel("Class Name")
    plt.title("Area and number of large, medium and small objects of each class")
    ...
    out_name = fig_set["out_name"]
    fig.savefig(
        f"{out_dir}/{out_name}_bbox_area.jpg", bbox_inches="tight", pad_inches=0.1
    )  # Save Image
    plt.close()
...
def main():
    ...
    def replace_pipeline_to_none(cfg):
        ...
        if cfg.get("dataset", None) is None and cfg.get("datasets", None) is None:
            return
    ...
    if args.class_name is None:
        ...
        raise RuntimeError(
            f"Expected args.class_name to be one of the list,"
    ...
    for index in range(len(dataset)):
        for instance in dataset[index]["instances"]:
            if instance["bbox_label"] in classes_idx and args.class_name is None:
        ...
        if len(bbox) > 0:
            ...
            for i in range(len(area_rule) - 1):
                bbox_area_nums[i] = np.logical_and(
                    bbox_area >= area_rule[i] ** 2, bbox_area < area_rule[i + 1] ** 2
                ).sum()
        elif len(bbox) == 0:
