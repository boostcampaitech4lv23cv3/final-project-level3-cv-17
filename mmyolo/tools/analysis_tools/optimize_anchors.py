...
def parse_args():
    ...
    parser.add_argument(
        "--out-dir", default=None, type=str, help="Path to save anchor optimize result."
    )
...
class BaseAnchorOptimizer:
    ...
    def save_result(self, anchors, path=None):
        ...
        for num in self.num_anchor_per_level:
            ...
            anchor_results.append([(round(w), round(h)) for w, h in anchors[start:end]])
            start = end
...
class YOLOV5KMeansAnchorOptimizer(BaseAnchorOptimizer):
    ...
    def optimize(self):
        ...
        bbox_whs = torch.from_numpy(self.bbox_whs).to(self.device, dtype=torch.float32)
        ...
        self.logger.info(
            f"{mean_matched:.2f} anchors/target {best_ratio:.3f} "
    ...
    def anchor_generate(
        ...
        if kmeans:
            try:
                ...
                anchors = (
                    kmeans(box_size.cpu().numpy() / sigmas, num, iter=30)[0] * sigmas
                )
                ...
                anchors = np.sort(np.random.rand(num * 2)).reshape(num, 2) * img_size
        else:
            self.logger.info(
    ...
    def anchor_metric(
        self, box_size: Tensor, anchors: Tensor, threshold: float = 4.0
    ) -> Tuple:
...
def main():
    ...
    assert (
        anchor_type == "mmdet.YOLOAnchorGenerator"
