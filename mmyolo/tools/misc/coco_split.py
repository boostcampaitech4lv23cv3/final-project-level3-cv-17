...
def parse_args():
    ...
    parser.add_argument("--json", type=str, required=True, help="COCO json label path")
...
def split_coco_dataset(
    coco_json_path: str, save_dir: str, ratios: list, shuffle: bool, seed: int
):
    ...
    if val_image_num != 0:
        val_image_ids = coco_image_ids[
            train_image_num : train_image_num + val_image_num
        ]
    else:
        val_image_ids = None
...
def main():
    args = parse_args()
    split_coco_dataset(args.json, args.out_dir, args.ratios, args.shuffle, args.seed)
