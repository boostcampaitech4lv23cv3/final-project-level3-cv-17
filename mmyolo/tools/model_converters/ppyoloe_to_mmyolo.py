    ...
    if imagenet_pretrain:
        for k, v in model.items():
            ...
            if "stem." in k:
                ...
                name = k.replace("stem.conv%s." % org_ind, "stem.%s." % new_ind)
            else:
                ...
                name = k.replace(
                    "stages.%s." % org_stage_ind, "stage%s.0." % new_stage_ind
                )
            ...
            if k.startswith("backbone."):
                ...
                if k.startswith("neck.fpn_stages."):
                    ...
                    if k.startswith("neck.fpn_stages.0.0."):
                        name = k.replace(
                            "neck.fpn_stages.0.0.", "neck.reduce_layers.2.0."
                        )
                        if ".spp." in name:
                    ...
                    # -> neck.top_down_layers.0.0.conv1.conv.weight
                        ...
                        name = k.replace(
                            "neck.fpn_stages.1.0.", "neck.top_down_layers.0.0."
                        )
                    elif k.startswith("neck.fpn_stages.2.0."):
                        name = k.replace(
                            "neck.fpn_stages.2.0.", "neck.top_down_layers.1.0."
                        )
                    else:
                ...
                if "proj_conv" in k:
                    name = k.replace(
                        "yolo_head.proj_conv.", "bbox_head.head_module.proj_conv."
                    )
                else:
                    for org_key, rep_key in [
                        ["yolo_head.stem_cls.", "bbox_head.head_module.cls_stems."],
                        ["yolo_head.stem_reg.", "bbox_head.head_module.reg_stems."],
                        ["yolo_head.pred_cls.", "bbox_head.head_module.cls_preds."],
                        ["yolo_head.pred_reg.", "bbox_head.head_module.reg_preds."],
                    ]:
...
def main():
    ...
    parser.add_argument("--dst", default="mmppyoloe_plus_s.pt", help="save path")
    parser.add_argument(
        "--imagenet-pretrain",
        action="store_true",
        default=False,
