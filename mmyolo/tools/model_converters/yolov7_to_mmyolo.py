...
def convert(src, dst):
    ...
    for key, weight in blobs.items():
        ...
        if int(num) < indexes[0] and int(num) not in indexes[1]:
            ...
            new_key = key.replace(".".join(strs_key), convert_dict[".".join(strs_key)])
            state_dict[new_key] = weight
            print(f"Convert {key} to {new_key}")
        else:
    ...
    for i in range(num_levels):
        if num_levels == 3:
            implicit_a = f"bbox_head.head_module." f"convs_pred.{i}.0.implicit"
            state_dict[implicit_a] = torch.zeros((1, in_channels[i], 1, 1))
            implicit_m = f"bbox_head.head_module." f"convs_pred.{i}.2.implicit"
            state_dict[implicit_m] = torch.ones((1, 3 * 85, 1, 1))
        else:
            implicit_a = f"bbox_head.head_module." f"main_convs_pred.{i}.1.implicit"
            state_dict[implicit_a] = torch.zeros((1, in_channels[i], 1, 1))
            implicit_m = f"bbox_head.head_module." f"main_convs_pred.{i}.3.implicit"
            state_dict[implicit_m] = torch.ones((1, 3 * 85, 1, 1))
...
def main():
    ...
    parser.add_argument("src", default="yolov7.pt", help="src yolov7 model path")
