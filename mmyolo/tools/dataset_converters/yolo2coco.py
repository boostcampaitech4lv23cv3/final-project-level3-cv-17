    ...
    for cat in categories:
        if cat in existed_categories:
            ...
            with open(txt_file) as f:
                ...
                img_paths = [
                    os.path.split(img_path.strip())[1] for img_path in img_paths
...
def convert_yolo_to_coco(image_dir: str):
    ...
    for idx, image in enumerate(mmengine.track_iter_progress(indices)):
        ...
        if not osp.exists(label_path):
            ...
            print(f"WARNING: {label_path} does not exist. Please check the file.")
            skipped += 1
            continue
    ...
    print(
        f"Number of images found: {total}, converted: {converted},",
