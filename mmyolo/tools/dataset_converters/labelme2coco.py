...
def format_coco_annotations(
    points: list, image_id: int, annotations_id: int, category_id: int
) -> dict:
    ...
    annotation_info["area"] = (
        annotation_info["bbox"][2] * annotation_info["bbox"][3]
    )  # bbox w * h
...
def parse_labelme_to_coco(
    image_dir: str, labels_root: str, all_classes_id: Optional[dict] = None
) -> (dict, dict):
        ...
        for label_shapes in labelme_data["shapes"]:
            ...
            if (all_classes_id is None) and (class_name not in categories_labels):
                ...
                coco_json["categories"].append(
                    {
                        "id": len(categories_labels) + 1,  # categories id start with 1
                        "name": class_name,
                    }
                )
            ...
            elif (all_classes_id is not None) and (class_name not in categories_labels):
                # check class name
                raise ValueError(
                    f"Got unexpected class name {class_name}, "
    ...
    print(
        f"Number of categories = {len(categories_labels)}, "
...
def convert_labelme_to_coco(
    image_dir: str, labels_dir: str, out_path: str, class_id_txt: Optional[str] = None
):
    ...
    if class_id_txt is None:
        ...
        print(
            "Not Saving new class id txt, user should using "
...
def main():
    args = parse_args()
    convert_labelme_to_coco(args.img_dir, args.labels_dir, args.out, args.class_id_txt)
    print("All done!")
