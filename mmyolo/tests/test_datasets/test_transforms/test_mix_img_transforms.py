...
class TestMosaic(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dataset = YOLOv5CocoDataset(
            data_prefix=dict(img=osp.join(osp.dirname(__file__), "../../data")),
            ann_file=osp.join(
    ...
    def test_transform(self):
        ...
        transform = Mosaic(img_scale=(12, 10), pre_transform=self.pre_transform)
...
class TestMosaic9(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        transform = Mosaic9(img_scale=(12, 10), pre_transform=self.pre_transform)
        results = transform(copy.deepcopy(self.results))
        self.assertTrue(results["img"].shape[:2] == (20, 24))
        self.assertTrue(
            results["gt_bboxes_labels"].shape[0] == results["gt_bboxes"].shape[0]
...
class TestYOLOv5MixUp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.results = {
            "img": np.random.random((288, 512, 3)),
            "img_shape": (288, 512),
            "gt_bboxes_labels": np.array([1, 2, 3], dtype=np.int64),
            "gt_bboxes": np.array(
                [[10, 10, 20, 20], [20, 20, 40, 40], [40, 40, 80, 80]], dtype=np.float32
            ...
            "gt_ignore_flags": np.array([0, 0, 1], dtype=bool),
            "gt_masks": BitmapMasks(rng.rand(3, 288, 512), height=288, width=512),
            "dataset": self.dataset,
        }
