...
class TestLetterResize(unittest.TestCase):
    ...
    def test_letter_resize(self):
        ...
        self.assertTrue(
            (results["gt_bboxes"] == np.array([[208.0, 0.0, 304.0, 96.0]])).all()
        )
        self.assertTrue((results["batch_shape"] == np.array([192, 672])).all())
        self.assertTrue(
            (results["pad_param"] == np.array([0.0, 0.0, 208.0, 208.0])).all()
        )
        self.assertTrue(
            (np.array(results["scale_factor"], dtype=np.float32) <= 1.0).all()
        )
        ...
        self.assertTrue(
            (results["gt_bboxes"] == np.array([[0.0, 0.0, 96.0, 96.0]])).all()
        )
        self.assertTrue((results["batch_shape"] == np.array([192, 672])).all())
        self.assertTrue((results["pad_param"] == np.array([0.0, 0.0, 0.0, 0.0])).all())
        self.assertTrue(
            (np.array(results["scale_factor"], dtype=np.float32) <= 1.0).all()
        )
        ...
        self.assertTrue(
            (
                results["gt_bboxes"] == np.array([[0.0, 0.0, 251.99998474121094, 96.0]])
            ).all()
        )
        self.assertTrue((results["batch_shape"] == np.array([192, 672])).all())
        self.assertTrue((results["pad_param"] == np.array([0.0, 0.0, 0.0, 0.0])).all())
        ...
        rng = np.random.RandomState(0)
        for _ in range(5):
            input_h, input_w = np.random.randint(100, 700), np.random.randint(100, 700)
            output_h, output_w = np.random.randint(100, 700), np.random.randint(
                100, 700
            )
            data_info = dict(
                ...
                gt_masks=BitmapMasks(
                    rng.rand(1, input_h, input_w), height=input_h, width=input_w
                ),
            )
            results = transform(data_info)
            self.assertEqual(results["img_shape"], (output_h, output_w, 3))
            self.assertTrue(
                (results["batch_shape"] == np.array([output_h, output_w])).all()
            )
        ...
        for _ in range(5):
            ...
            pad_param = (
                data_info["pad_param"].reshape(-1, 2).sum(1)
            )  # (top, b, l, r) -> (h, w)
            scale_factor = np.asarray(data_info["scale_factor"])[
                ::-1
            ]  # (w, h) -> (h, w)
            scale_factor_keepratio = np.min(np.asarray((32, 32)) / (input_h, input_w))
            validate_shape = np.floor(
                np.asarray((input_h, input_w)) * scale_factor_keepratio + 0.5
            )
            scale_factor_keepratio = (
                np.floor(scale_factor_keepratio * input_h + 0.5) / input_h
            )
            scale_factor_letter = (output_h, output_w) / validate_shape
            scale_factor_letter = (scale_factor_letter - (pad_param / validate_shape))[
                np.argmin(scale_factor_letter)
            ]
            self.assertTrue(data_info["img_shape"][:2] == (output_h, output_w))
            self.assertTrue(
                (scale_factor == (scale_factor_keepratio * scale_factor_letter)).all()
            )
...
class TestYOLOv5KeepRatioResize(unittest.TestCase):
    ...
    def test_yolov5_keep_ratio_resize(self):
        ...
        self.assertTrue(
            (results["gt_bboxes"] == np.array([[0.0, 0.0, 240.0, 240.0]])).all()
        )
        self.assertTrue(
            (np.array(results["scale_factor"], dtype=np.float32) == 1.6).all()
        )
...
class TestYOLOv5HSVRandomAug(unittest.TestCase):
    ...
    def test_yolov5_hsv_random_aug(self):
        ...
        self.assertTrue(results["img"].shape[:2] == self.data_info["img"].shape[:2])
...
class TestLoadAnnotations(unittest.TestCase):
    ...
    def test_load_bboxes(self):
        ...
        self.assertTrue(
            (
                results["gt_bboxes"] == np.array([[0, 0, 10, 20], [10, 10, 110, 120]])
            ).all()
        )
        self.assertEqual(results["gt_bboxes"].dtype, np.float32)
        self.assertTrue((results["gt_ignore_flags"] == np.array([False, False])).all())
    ...
    def test_load_labels(self):
        ...
        self.assertTrue((results["gt_bboxes_labels"] == np.array([1, 2])).all())
...
class TestPPYOLOERandomDistort(unittest.TestCase):
    ...
    def test_transform(self):
        ...
        with self.assertRaises(AssertionError):
            transform = PPYOLOERandomDistort(hue_cfg=dict(min=-18, max=18, prob=1.5))
