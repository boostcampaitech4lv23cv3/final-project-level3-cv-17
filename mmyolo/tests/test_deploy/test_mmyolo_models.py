...
def test_yolov5_head_predict_by_feat(backend_type: Backend):
    ...
    cls_scores = [
        torch.rand(
            1,
            yolov5_head.num_classes * yolov5_head.num_base_priors,
            4 * pow(2, i),
            4 * pow(2, i),
        )
    ...
    bbox_preds = [
        torch.rand(1, 4 * yolov5_head.num_base_priors, 4 * pow(2, i), 4 * pow(2, i))
        for i in range(3, 0, -1)
    ]
    seed_everything(9101)
    objectnesses = [
        torch.rand(1, 1 * yolov5_head.num_base_priors, 4 * pow(2, i), 4 * pow(2, i))
        for i in range(3, 0, -1)
    ]
    ...
    model_outputs = get_model_outputs(yolov5_head, "predict_by_feat", model_inputs)

    # to get outputs of onnx model after rewrite
    wrapped_model = WrapModel(
        yolov5_head, "predict_by_feat", batch_img_metas=batch_img_metas, with_nms=True
    )
    rewrite_inputs = {
        "cls_scores": cls_scores,
        "bbox_preds": bbox_preds,
    ...
    rewrite_outputs, is_backend_output = get_rewrite_outputs(
        wrapped_model=wrapped_model, model_inputs=rewrite_inputs, deploy_cfg=deploy_cfg
    )
    ...
    if is_backend_output:
        ...
        min_shape = min(
            model_outputs[0].bboxes.shape[0], rewrite_outputs[0].shape[1], 5
        )
        for i in range(len(model_outputs)):
            rewrite_outputs[0][i, :min_shape, 0::2] = rewrite_outputs[0][
                i, :min_shape, 0::2
            ].clamp_(0, s)
            rewrite_outputs[0][i, :min_shape, 1::2] = rewrite_outputs[0][
                i, :min_shape, 1::2
            ].clamp_(0, s)
            assert np.allclose(
