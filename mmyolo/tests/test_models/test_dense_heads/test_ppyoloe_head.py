...
class TestPPYOLOEHead(TestCase):
    ...
    def test_init_weights(self):
        ...
        for conv_cls, conv_reg in zip(
            head.head_module.cls_preds, head.head_module.reg_preds
        ):
            assert_allclose(
                conv_cls.weight.data, torch.zeros_like(conv_cls.weight.data)
            )
            assert_allclose(
                conv_reg.weight.data, torch.zeros_like(conv_reg.weight.data)
            )

            assert_allclose(
                conv_cls.bias.data, torch.ones_like(conv_cls.bias.data) * bias_init
            )
            assert_allclose(conv_reg.bias.data, torch.ones_like(conv_reg.bias.data))
    ...
    def test_loss_by_feat(self):
        ...
        for i in range(len(self.head_module["in_channels"])):
            ...
            feat.append(torch.rand(1, in_channel, s // feat_size, s // feat_size))
        ...
        empty_gt_losses = head.loss_by_feat(
            cls_scores, bbox_preds, bbox_dist_preds, gt_instances, img_metas
        )
        ...
        head.train()
        gt_instances = torch.Tensor([[0.0, 0.0, 23.6667, 23.8757, 238.6326, 151.8874]])
        ...
        self.assertGreater(onegt_loss_dfl.item(), 0, "obj loss should be non-zero")
