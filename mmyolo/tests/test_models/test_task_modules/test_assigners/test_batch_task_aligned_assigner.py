...
class TestBatchTaskAlignedAssigner(TestCase):
    ...
    def test_batch_task_aligned_assigner(self):
        ...
        gt_labels = torch.LongTensor([[0], [1]]).unsqueeze(0).repeat(batch_size, 1, 1)
        pad_bbox_flag = (
            torch.FloatTensor([[1], [0]]).unsqueeze(0).repeat(batch_size, 1, 1)
        )
        pred_bboxes = (
            torch.FloatTensor(
                [
        ...
        self.assertEqual(assigned_bboxes.shape, torch.Size([batch_size, 84, 4]))
        self.assertEqual(
            assigned_scores.shape, torch.Size([batch_size, 84, num_classes])
        )
